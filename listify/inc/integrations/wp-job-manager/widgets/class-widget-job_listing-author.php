...
class Listify_Widget_Listing_Author extends Listify_Widget {
	...
	public function get_avatar( $instance ) {
		...
		$post = get_post();
		$image = isset( $instance[ 'image' ] ) ? $instance[ 'image' ] : 'avatar';
...
<div class="job_listing-author-join-date">
	<?php printf( __( 'Member since %s', 'listify' ), date( 'F Y', strtotime( $join_date ) ) ); ?>
</div>
...
<?php
	...
	public function the_contact_button( $instance ) {
		...
		if ( listify_has_integration( 'private-messages' ) ) {
?>

<a href="<?php echo esc_url( pm_get_new_message_url( $this->listing_owner, apply_filters( 'listify_private_message_default_subject', '' ), apply_filters( 'listify_private_message_default_message', '' ) ) ); ?>" class="button button-secondary">
	<?php echo esc_attr( $contact_text ); ?>
</a>

<?php
		} elseif ( '' != $post->_application ) { 
?>
