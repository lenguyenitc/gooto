...
class Listify_Setup {
	...
	public static function includes() {
		// @see https://github.com/astoundify/setup-guide
		include_once( get_template_directory() . '/vendor/astoundify/setup-guide/app/class-astoundify-setup-guide.php' );

		include_once( dirname( __FILE__ ) . '/lib/astoundify-contentimporter/ContentImporter.php' );
		include_once( dirname( __FILE__ ) . '/lib/astoundify-themeforestupdater/class-astoundify-themeforest-updater.php' );
		include_once( dirname( __FILE__ ) . '/lib/use-child-theme/use-child-theme.php' );
	}
...
<?php
	...
	public static function content_importer() {
		...
		Astoundify_ContentImporter::set_url( get_template_directory_uri() . '/inc/setup/lib/astoundify-contentimporter' );
	...
	public static function _content_importer_stage() {
		...
		if ( ! is_wp_error( $importer ) ) {
			...
			if ( 0 === count( $importer->get_items() ) ) {
				return wp_send_json_error( __( 'Cannot read files on system. Try changing your <code>FS_METHOD</code> to <code>direct</code>. <a href="https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants">More information</a>', 'listify' ) );
			}
