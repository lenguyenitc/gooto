...
class Listify_Widget_Author_Listings extends Listify_Widget {
    ...
    public function __construct() {
        ...
        /* Use theme mod to store per page option */
        add_action( 'listify_widget_update_after', array( $this, 'update_author_per_page' ), 10, 3 );
    }

	/**
	 * Update Author Per Page Option
	 * @link https://github.com/Astoundify/listify/issues/1526
	 */
	public function update_author_per_page( $instance, $new_instance, $this_widget ){
		if( 'listify_widget_author_listings' == $this_widget->widget_id && isset( $new_instance['per_page'] ) ){
			set_theme_mod( '_listify_author_archive_per_page', absint( $new_instance['per_page'] ) );
		}
	}
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $title ) {
			$title = str_replace( 
				...
				array( get_the_author_meta( 'display_name', get_queried_object_id() ), $wp_query->found_posts ),
				$title
			);
